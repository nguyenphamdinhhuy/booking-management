...
<h1 class="page-title">
    ...
    Chỉnh sửa loại phòng: <?php echo e($roomType->type_name); ?>

</h1>

<!-- Edit Room Type Form -->
<div class="content-section">
    <form action="<?php echo e(route('admin.roomType.update', $roomType->rt_id)); ?>" enctype="multipart/form-data" method="POST" class="room-form" id="editRoomTypeForm">
        <?php echo csrf_field(); ?>
        ...
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <input type="text"
                    id="type_name"
                    name="type_name"
                    class="form-input <?php echo e($errors->has('type_name') ? 'error' : ''); ?>"
                    value="<?php echo e(old('type_name', $roomType->type_name)); ?>"
            ...
            <div class="form-group">
                ...
                <input type="number"
                    id="base_price"
                    name="base_price"
                    class="form-input <?php echo e($errors->has('base_price') ? 'error' : ''); ?>"
                    value="<?php echo e(old('base_price', $roomType->base_price)); ?>"
                    placeholder="500000"
                    min="0"
                    step="1000">
            ...
            <div class="form-group">
                ...
                <select id="max_guests"
                    ...
                    <?php for($i = 1; $i <= 20; $i++): ?>
                        <option value="<?php echo e($i); ?>" <?php echo e(old('max_guests', $roomType->max_guests) == $i ? 'selected' : ''); ?>>
                        <?php echo e($i); ?> khách
                        </option>
                        <?php endfor; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <select id="number_beds"
                    ...
                    <?php for($i = 1; $i <= 10; $i++): ?>
                        <option value="<?php echo e($i); ?>" <?php echo e(old('number_beds', $roomType->number_beds) == $i ? 'selected' : ''); ?>>
                        <?php echo e($i); ?> giường
                        </option>
                        <?php endfor; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text"
                    id="room_size"
                    name="room_size"
                    class="form-input <?php echo e($errors->has('room_size') ? 'error' : ''); ?>"
                    value="<?php echo e(old('room_size', $roomType->room_size)); ?>"
                    placeholder="VD: 30m², 45 m2..."
                    maxlength="50">
                <?php if($errors->has('room_size')): ?>
                <div class="error-message">
                    <i class="fas fa-exclamation-circle"></i>
                    <?php echo e($errors->first('room_size')); ?>
            ...
            <div class="form-group">
                ...
                <select id="status"
                    ...
                    <option value="1" <?php echo e(old('status', $roomType->status) == '1' ? 'selected' : ''); ?>>Hoạt động</option>
                    <option value="0" <?php echo e(old('status', $roomType->status) == '0' ? 'selected' : ''); ?>>Không hoạt động</option>
                </select>
            ...
            <div class="form-group full-width">
                ...
                <div id="amenityPicker"
                    ...
                    data-selected='<?php echo json_encode(old("amenities", $currentAmenities), 512) ?>'>
            ...
            <div class="form-group full-width">
                ...
                <div class="current-images-grid">
                    ...
                    ?>
                    <?php $__currentLoopData = $currentImages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $image): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="current-image-item">
                        <img src="<?php echo e(asset($image)); ?>" alt="Room image <?php echo e($index + 1); ?>">
                        <div class="image-name"><?php echo e(basename($image)); ?></div>
                    </div>
            ...
            <div class="form-group full-width">
                <label for="images" class="form-label">
                    ...
                    <?php echo e($roomType->images ? 'Thay thế hình ảnh' : 'Thêm hình ảnh loại phòng'); ?>
            ...
            <div class="form-group full-width">
                ...
                <textarea id="description"
                    ...
                    rows="5"><?php echo e(old('description', $roomType->description)); ?></textarea>
...
<script>
    ...
    const originalFormData = {
        type_name: "<?php echo e($roomType->type_name); ?>",
        base_price: "<?php echo e($roomType->base_price); ?>",
        max_guests: "<?php echo e($roomType->max_guests); ?>",
        number_beds: "<?php echo e($roomType->number_beds); ?>",
        room_size: "<?php echo e($roomType->room_size ?? ''); ?>",
        status: "<?php echo e($roomType->status); ?>",
        description: "<?php echo e($roomType->description ?? ''); ?>",
        amenities: <?php echo json_encode($currentAmenities, 15, 512) ?>
    };
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function saveDraft() {
            ...
            if (hasChanges) {
                localStorage.setItem('roomTypeEditDraft_<?php echo e($roomType->rt_id); ?>', JSON.stringify(formData));
            }
        }

        // Load draft on page load
        function loadDraft() {
            const draftKey = 'roomTypeEditDraft_<?php echo e($roomType->rt_id); ?>';
        ...
        window.addEventListener('beforeunload', function() {
            ...
            if (document.querySelector('.alert-success')) {
                localStorage.removeItem('roomTypeEditDraft_<?php echo e($roomType->rt_id); ?>');
            }
        });
    });
    ...
    (function() {
        ...
        function bindDeleteButtons() {
            listEl.querySelectorAll('button.amenity-delete').forEach(btn => {
                btn.addEventListener('click', async () => {
                    ...
                    hideAmenityError();
                    try {
                        const res = await fetch(`<?php echo e(route('admin.amenities.delete')); ?>`, {
                            method: 'DELETE',
                            headers: {
        ...
        async function loadAmenities() {
            hideAmenityError();
            try {
                const res = await fetch(`<?php echo e(route('admin.amenities.index')); ?>`, {
                    headers: {
                        'Accept': 'application/json'
                    }
                });
        ...
        async function onAddAmenity() {
            ...
            hideAmenityError();
            try {
                const res = await fetch(`<?php echo e(route('admin.amenities.store')); ?>`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': CSRF,
...
<?php echo $__env->make('admin.layouts.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\F-poly\graduation_project\booking_laravel_project\booking_app - Copy\resources\views/admin/room_types/edit.blade.php ENDPATH**/ ?>