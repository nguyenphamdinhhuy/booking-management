                            ...
                            <div class="staff-grid">
                                @foreach ($staffs as $staff)
                                    <div class="staff-card">
                                        <img src="{{ asset($staff->avatar) }}" alt="avatar">
                                        <div class="staff-info">
                                            <h4>{{ $staff->name }}</h4>
                                            <p><strong>Email:</strong> {{ $staff->email }}</p>
                                            <p><strong>Giới tính:</strong> {{ $staff->gender }}</p>
                                            <p><strong>Ngày sinh:</strong> {{ \Carbon\Carbon::parse($staff->dob)->format('d/m/Y') }}</p>
                                            <p><strong>Chức vụ:</strong> {{ $staff->position ?? 'Chính thức' }}</p>
                                            <p><strong>Lương:</strong> {{ number_format($staff->salary, 0, ',', '.') }} VNĐ</p>
                                            <a href="javascript:void(0)" class="btn-detail" data-id="{{ $staff->id }}">Xem chi tiết</a>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
