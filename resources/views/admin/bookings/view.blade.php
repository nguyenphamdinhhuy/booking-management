...
<div class="booking-detail-container">
    ...
    <div class="urgency-indicator {{ $daysUntilCheckIn <= 1 ? 'urgent' : '' }}">
        ...
        <div>
            ...
            @if($daysUntilCheckIn <= 0)
                ...
                @else
                Còn {{ $daysUntilCheckIn }} ngày đến ngày nhận phòng. Cần chọn phòng cho khách hàng.
                @endif
                </div>
        </div>
        @endif
        ...
        <div class="checkin-ready-indicator">
            ...
            <div>
                ...
                Phòng {{ $booking->assigned_room_name }} đã được chọn. Khách hàng có thể nhận phòng từ ngày {{ \Carbon\Carbon::parse($booking->check_in_date)->format('d/m/Y') }}.
        ...
        <div class="booking-header">
            ...
            <div class="booking-id">Mã đặt phòng: #{{ $booking->b_id }}</div>
            <div class="status-badge 
        @if($booking->status == 0) status-pending
        @elseif($booking->status == 1) status-waiting-admin  
        @elseif($booking->status == 2) status-confirmed
        @elseif($booking->status == 3) status-checkedin
        @elseif($booking->status == 4) status-completed
        ...
        <div class="booking-content">
            ...
            <div class="main-info">
                <div class="room-type-info">
                    <!-- Tên loại phòng -->
                    <h2 class="room-type-name">{{ $booking->room_type_name ?? 'Loại phòng không xác định' }}</h2>

                    <div class="room-type-features">
                        <div class="feature">
                            <span class="feature-icon">👥</span>
                            <span>Tối đa {{ $booking->max_guests ?? 'N/A' }} khách</span>
                        </div>
                        <div class="feature">
                            <span class="feature-icon">🛏️</span>
                            <span>{{ $booking->number_beds ?? 'N/A' }} giường</span>
                        </div>
                        <div class="feature">
                            <span class="feature-icon">📏</span>
                            <span>{{ $booking->room_size ?? 'N/A' }}</span>
                        </div>
                    </div>

                    <!-- Mô tả loại phòng -->
                    @if($booking->room_type_description)
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin-top: 15px;">
                        <strong>Mô tả loại phòng:</strong>
                        <p style="margin: 8px 0 0 0; color: #666;">{{ $booking->room_type_description }}</p>
                    </div>
                    @endif
                    ...
                    <div class="adbks-svcard">
                        ...
                        <div class="adbks-svlist">
                            ...
                            <div class="adbks-svrow">
                                <div class="adbks-svrow__left">
                                    <div class="adbks-svrow__name">{{ $svc->service_name }}</div>
                                    <div class="adbks-svrow__meta">
                                        <span class="adbks-chip">{{ $svc->pricing_label }}</span>
                                        @if($svc->scheduled_fmt)
                                        <span class="adbks-dot">•</span>
                                        <span class="adbks-muted">Lịch: {{ $svc->scheduled_fmt }}</span>
                                        @endif
                                        @if($svc->location)
                                        <span class="adbks-dot">•</span>
                                        <span class="adbks-muted">Khu vực: {{ $svc->location }}</span>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="adbks-svrow__right">
                                    <div class="adbks-svrow__unit">{{ $svc->unit_price_fmt }}</div>
                                    <div class="adbks-svrow__sum">{{ $svc->subtotal_fmt }}</div>
                                </div>
                            </div>
                            @endforeach
                        </div>
                        ...
                        <div class="adbks-svtotal">
                            ...
                            <strong>{{ $services_total_fmt }}</strong>
                        </div>
                        @else
                    ...
                    <div style="background: #e8f4fd; padding: 15px; border-radius: 8px; margin-top: 15px;">
                        ...
                        <p style="margin: 8px 0 0 0; color: #666;">{{ $booking->booking_description }}</p>
                    </div>
                    @endif
                </div>
                ...
                <div class="room-assignment-section">
                    ...
                    <div class="current-assignment">
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div>
                                <strong style="font-size: 16px;">{{ $booking->assigned_room_name }}</strong>
                                <div style="font-size: 14px; color: #666; margin-top: 2px;">
                                    Phòng số {{ $booking->assigned_room_name }} - {{ $booking->room_type_name }}
                                </div>
                            </div>
                            @if($booking->status == 1)
                            <form method="POST" action="{{ route('admin.bookings.confirm.room', $booking->b_id) }}" style="display: inline;">
                                @csrf
                    ...
                    <div class="assignment-form">
                        <form method="POST" action="{{ route('admin.bookings.assign.room', $booking->b_id) }}">
                            @csrf
                            <div class="form-group">
                                <label class="form-label">
                                    <i class="fas fa-list"></i>
                                    Chọn phòng trống cho loại phòng "{{ $booking->room_type_name }}"
                                </label>
                                <select name="room_id" class="form-select" onchange="showRoomDetails(this)" required>
                                    ...
                                    <option value="{{ $room->r_id }}"
                                        ...
                                        data-price="{{ number_format($room->price_per_night, 0, ',', '.') }}"
                                        {{ $booking->r_id_assigned == $room->r_id ? 'selected' : '' }}>
                                        Phòng {{ $room->name }} - {{ number_format($room->price_per_night, 0, ',', '.') }} VNĐ/đêm
                                    </option>
                                    @endforeach
                                </select>
                                ...
                                <div style="background: #f8d7da; color: #721c24; padding: 10px; border-radius: 6px; margin-top: 10px;">
                                    ...
                                    Tất cả phòng loại "{{ $booking->room_type_name }}" đều đã được đặt trong khoảng thời gian này.
                                </div>
                                @endif
                            </div>
                            ...
                            <div style="display: flex; gap: 10px; justify-content: flex-end;">
                                <button type="submit" class="btn btn-primary"
                                    ...
                                    <i class="fas fa-save"></i> {{ $booking->r_id_assigned ? 'Cập nhật phòng' : 'Chọn phòng' }}
                                </button>
                            </div>
                            @endif
                        </form>
                    ...
                    <div style="background: #e8f4fd; padding: 15px; border-radius: 8px;">
                        <div style="display: flex; align-items: center; gap: 10px;">
                            ...
                            <div>
                                <strong>Đã chọn phòng {{ $booking->assigned_room_name }}</strong>
                    ...
                    <div style="background: #d4edda; padding: 15px; border-radius: 8px;">
                        <div style="display: flex; align-items: center; gap: 10px;">
                            ...
                            <div>
                                <strong>Phòng {{ $booking->assigned_room_name }} đã được giao</strong>
                                <div style="font-size: 14px; color: #666; margin-top: 5px;">
                ...
                <div class="booking-dates">
                    <div class="date-grid">
                        <div class="date-item">
                            ...
                            <div class="date-value">{{ \Carbon\Carbon::parse($booking->check_in_date)->format('d/m/Y') }}</div>
                            <div style="font-size: 12px; color: #666; margin-top: 4px;">15:00</div>
                        </div>
                        <div class="date-item">
                            <div class="date-label">Trả phòng</div>
                            <div class="date-value">{{ \Carbon\Carbon::parse($booking->check_out_date)->format('d/m/Y') }}</div>
        ...
        <div class="booking-summary">
            ...
            <div class="summary-item">
                ...
                <span class="summary-value">{{ $booking->user_name }}</span>
            </div>

            <div class="summary-item">
                <span class="summary-label">Email</span>
                <span class="summary-value">{{ $booking->user_email }}</span>
            </div>

            <div class="summary-item">
                <span class="summary-label">Loại phòng</span>
                <span class="summary-value">{{ $booking->room_type_name }}</span>
            </div>

            <div class="summary-item">
                <span class="summary-label">Giá cơ bản/đêm</span>
                <span class="summary-value">{{ number_format($booking->base_price, 0, ',', '.') }} VNĐ</span>
            </div>

            <div class="summary-item">
                <span class="summary-label">Số đêm</span>
                <span class="summary-value">{{ $nights }} đêm</span>
            </div>

            <div class="summary-item">
                <span class="summary-label">Ngày đặt</span>
                <span class="summary-value">{{ \Carbon\Carbon::parse($booking->created_at)->format('d/m/Y H:i') }}</span>
            </div>
            ...
            <div class="summary-item">
                ...
                <span class="summary-value">{{ $room_only_total_fmt }}</span>
            </div>

            <div class="summary-item">
                <span class="summary-label">Dịch vụ kèm theo</span>
                <span class="summary-value">{{ $services_total_fmt }}</span>
            </div>

            <div class="summary-item">
                <span class="summary-label">Tổng tiền</span>
                <span class="summary-value">{{ number_format($booking->total_price, 0, ',', '.') }} VNĐ</span>
            </div>

            <!-- Action buttons -->
            <div class="action-buttons">
                {{-- Xác nhận phòng đã chọn --}}
                @if($booking->status == 1 && $booking->r_id_assigned)
                <form method="POST" action="{{ route('admin.bookings.confirm.room', $booking->b_id) }}">
                    @csrf
                    <button type="submit" class="btn btn-success"
                        onclick="return confirm('Xác nhận phòng {{ $booking->assigned_room_name }} và chuyển trạng thái sang chờ nhận phòng?')">
                ...
                {{-- Hủy hoặc Hoàn tiền (theo trạng thái thanh toán) --}}
                @if(in_array($booking->status, [1,2]))
                @if($booking->payment_status == 1)
                {{-- ĐÃ thanh toán → Vô hiệu hóa Hủy, hiển thị Hoàn tiền --}}
                ...
                <form method="POST" action="{{ route('admin.bookings.refund', $booking->b_id) }}">
                    @csrf
                    <button type="submit" class="btn btn-warning"
                        onclick="return confirm('Xác nhận hoàn tiền cho đơn #{{ $booking->b_id }}?');">
                        <i class="fas fa-undo"></i> Hoàn tiền
                    </button>
                </form>
                @else
                {{-- CHƯA thanh toán → Cho phép Hủy --}}
                <a href="{{ route('admin.bookings.cancel.form', $booking->b_id) }}" class="btn btn-danger">
