...
<div class="vouchers-page">
    ...
    <div class="vouchers-header">
        <div class="header-content">
            ...
            <div class="header-stats">
                <div class="stat-item">
                    <div class="stat-number">{{ $all_vouchers->count() }}</div>
                    <div class="stat-label">Ưu đãi khả dụng</div>
                </div>
                <div class="stat-item">
                    <div class="stat-number">{{ $featured_vouchers->count() }}</div>
    ...
    <div class="vouchers-filters">
        <div class="filters-container">
            <div class="search-box">
                <form method="GET" action="{{ route('user.vouchers') }}">
                    <div class="search-input-wrapper">
                        <i class="fas fa-search"></i>
                        <input type="text" name="search" placeholder="Tìm kiếm mã giảm giá..." value="{{ request('search') }}">
            ...
            <div class="filter-tabs">
                <a href="{{ route('user.vouchers') }}" class="filter-tab {{ !request('type') ? 'active' : '' }}">
                    <i class="fas fa-star"></i> Tất cả
                </a>
                <a href="{{ route('user.vouchers', ['type' => 'hotel']) }}" class="filter-tab {{ request('type') == 'hotel' ? 'active' : '' }}">
                    <i class="fas fa-bed"></i> Khách sạn
                </a>
                <a href="{{ route('user.vouchers', ['type' => 'service']) }}" class="filter-tab {{ request('type') == 'service' ? 'active' : '' }}">
    ...
    <div class="featured-section">
        ...
        <div class="featured-vouchers">
            @foreach($featured_vouchers as $voucher)
            <div class="featured-voucher">
                ...
                <div class="voucher-discount">
                    <div class="discount-percent">{{ $voucher->discount_percent }}%</div>
                ...
                <div class="voucher-content">
                    <div class="voucher-code">
                        ...
                        <span class="code-value">{{ $voucher->v_code }}</span>
                    </div>

                    <div class="voucher-description">
                        {{ $voucher->description ?: 'Giảm giá ' . $voucher->discount_percent . '% cho đơn hàng của bạn' }}
                    </div>

                    <div class="voucher-validity">
                        <i class="fas fa-calendar-alt"></i>
                        <span>Có hiệu lực đến: {{ \Carbon\Carbon::parse($voucher->end_date)->format('d/m/Y') }}</span>
                    </div>

                    <div class="voucher-actions">
                        <button class="copy-btn" onclick="copyVoucherCode('{{ $voucher->v_code }}', {{ $voucher->v_id }})">
                            <i class="fas fa-copy"></i>
                            <span>Sao chép mã</span>
                        </button>
                        <a href="{{ route('all_rooms') }}" class="use-btn">
    ...
    <div class="all-vouchers-section">
        <div class="section-header">
            <h2>Tất cả ưu đãi</h2>
            <p>{{ $all_vouchers->count() }} ưu đãi khả dụng</p>
        </div>

        @if($all_vouchers->count() > 0)
        <div class="vouchers-grid">
            @foreach($all_vouchers as $voucher)
            <div class="voucher-card">
                <div class="voucher-card-header">
                    <div class="discount-badge">
                        <span class="discount-value">{{ $voucher->discount_percent }}%</span>
            ...
            <div class="voucher-card-body">
                <div class="voucher-title">
                    ...
                    <span>Mã: {{ $voucher->v_code }}</span>
                </div>

                <div class="voucher-desc">
                    {{ $voucher->description ?: 'Giảm ' . $voucher->discount_percent . '% cho đơn hàng của bạn' }}
                </div>
                ...
                <div class="voucher-details">
                    ...
                    <div class="detail-item">
                        ...
                        <span>Đến: {{ \Carbon\Carbon::parse($voucher->end_date)->format('d/m/Y') }}</span>
                    </div>
                </div>
            </div>

            <div class="voucher-card-footer">
                <button class="copy-button" onclick="copyVoucherCode('{{ $voucher->v_code }}', {{ $voucher->v_id }})">
                    <i class="fas fa-copy"></i>
                    <span>Sao chép</span>
                </button>
                <a href="{{ route('all_rooms') }}" class="use-button">
                    <i class="fas fa-arrow-right"></i>
                    <span>Sử dụng</span>
                </a>
            </div>
    ...
    <div class="empty-state">
        ...
        <a href="{{ route('user.vouchers') }}" class="btn-primary">Xem tất cả ưu đãi</a>
    </div>
    @endif
</div>
...
<script>
    ...
    async function copyVoucherCode(voucherCode, voucherId) {
        ...
        try {
            ...
            const response = await fetch('{{ route("vouchers.copy") }}', {
                method: 'POST',
                headers: {
