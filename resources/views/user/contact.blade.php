    ...
    <div class="contact-container">
        ...
        <div class="contact-wrapper">
            ...
            <div class="contact-map">
                <img src="{{ asset('images/lienhe.png') }}" alt="Bản đồ liên hệ" width="100%" height="400"
            ...
            <div class="contact-form">
                ...
                <form action="{{ route('contact.store') }}" method="POST" class="form-group">
                    @csrf
                    <div class="form-row">
                        <input type="text" name="name" placeholder="{{ Auth::user()->name }}" class="form-input" disabled>
                        <input type="text" name="phone" placeholder="Số điện thoại" class="form-input" required>
                    </div>

                    <input type="email" name="email" placeholder="{{ Auth::user()->email }}" class="form-input full-width"
                        disabled>
            ...
            <div class="chat-messages" id="chatMessages">
                @foreach($messages as $msg)
                    <div class="chat-message {{ $msg['type'] }}">
                        ...
                        {{ $msg['message'] }}<br>
                        <small>{{ \Carbon\Carbon::parse($msg['created_at'])->format('d/m/Y H:i') }}</small>
                    </div>
                @endforeach
            </div>
            ...
            $('#chatForm').on('submit', function (e) {
                ...
                $.ajax({
                    ...
                    data: {
                        _token: "{{ csrf_token() }}",
                        message: message,
                        subject: "Tin nhắn trò chuyện", // Mặc định để khỏi nhập
                        phone: "N/A" // Dùng giá trị mặc định, không bắt nhập
                    },
                    success: function (response) {
                        // Sau khi gửi thành công, thêm tin nhắn vào khung chat ngay
                        $('#chatMessages').append(`
                        ...
                        $('#chatMessage').val(''); // Xóa nội dung trong textarea
