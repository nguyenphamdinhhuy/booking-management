{{-- resources/views/user/room_type_detail.blade.php --}}
@extends('layouts.app')

@section('title', $roomType->type_name . ' - Chi tiết loại phòng')
...
<div class="room-detail-container">
    <nav class="breadcrumb">
        ...
        <a href="{{ route('all_rooms') }}">Loại phòng</a> &gt;
        {{ $roomType->type_name }}
    </nav>

    @if(session('error'))
    <div class="custom-alert-error">{{ session('error') }}</div>
    @endif
    @if(session('success'))
    ...
    <div class="room-header">
        <h1 class="room-title">{{ $roomType->type_name }}</h1>
        <p class="room-subtitle">{{ $roomType->room_size ? 'Diện tích: ' . $roomType->room_size : 'Phòng cao cấp' }}</p>

        @if($totalReviews > 0)
        <div class="rating-section">
            <div class="rating-badge">{{ $averageRating }}/5</div>
    ...
    <div class="main-content">
        ...
        <div class="room-info">
            ...
            <div class="gallery-section">
                ...
                <img src="{{ $imageUrls[0] }}" alt="{{ $roomType->type_name }}" class="main-image" id="mainImage"
                     onerror="this.onerror=null;this.src='{{ asset('assets/images/default-room.jpg') }}';">

                @if(count($imageUrls) > 1)
                <div class="image-thumbnails">
                    @foreach($imageUrls as $i => $url)
                    <img src="{{ $url }}" alt="Ảnh {{ $i+1 }}" class="thumbnail {{ $i===0 ? 'is-active' : '' }}"
                         data-src="{{ $url }}"
                         onerror="this.onerror=null;this.src='{{ asset('assets/images/default-room.jpg') }}';">
                    @endforeach
                </div>
            ...
            <div class="info-section">
                ...
                <div class="features-grid">
                    <div class="feature-item"><span class="feature-icon">👥</span><span class="feature-text">Tối đa {{ $roomType->max_guests }} khách</span></div>
                    <div class="feature-item"><span class="feature-icon">🛏️</span><span class="feature-text">{{ $roomType->number_beds }} giường</span></div>
                    @if($roomType->room_size)
                    <div class="feature-item"><span class="feature-icon">📏</span><span class="feature-text">{{ $roomType->room_size }}</span></div>
                    @endif
                    <div class="feature-item"><span class="feature-icon">🏨</span><span class="feature-text">{{ $availableRooms }} phòng có sẵn</span></div>
                </div>
            </div>
            ...
            <div class="info-section">
                ...
                <div class="amenities-list">
                    ...
                    <div class="amenity-item"><span class="amenity-icon">✓</span><span>{{ $amenity }}</span></div>
                    @endforeach
                </div>
            </div>
            @endif
            ...
            <div class="info-section services-section">
                ...
                <div class="service-item" data-sid="{{ $svc->s_id }}" data-price="{{ (int)$svc->price }}">
                    <div class="service-checkbox">
                        <input type="checkbox" class="svc-check" id="svc_{{ $svc->s_id }}">
                        <label for="svc_{{ $svc->s_id }}" class="service-name">{{ $svc->name }}</label>
                    </div>
                    <div class="service-price">{{ number_format((int)$svc->price, 0, ',', '.') }}₫</div>
                    <div class="service-qty">
                        <label for="qty_{{ $svc->s_id }}" style="font-size:12px;color:#666;">Số lượng</label>
                        <input type="number" id="qty_{{ $svc->s_id }}" class="svc-qty" value="1" min="1" step="1" disabled>
                    </div>
                </div>
                @endforeach
        ...
        <div class="booking-card">
            <div class="price-section">
                <span class="price">{{ number_format($roomType->base_price, 0, ',', '.') }} VNĐ</span>
                <span class="price-unit">/ đêm</span>
            </div>

            {{-- Services total + tạm tính --}}
            ...
            <div class="price-row" id="mini-total-row" style="display:flex;justify-content:space-between;margin:4px 0 16px;font-weight:700;">
                <span>Tạm tính</span>
                <span id="mini-order-total">{{ number_format($roomType->base_price, 0, ',', '.') }}₫</span>
            </div>

            <div class="availability-status" id="availabilityStatus"></div>

            <form class="booking-form" method="GET" action="{{ route('room-type.validate-booking') }}" onsubmit="return validateBookingForm();">
                <input type="hidden" name="room_type_id" value="{{ $roomType->rt_id }}">

                {{-- nơi chứa hidden inputs dịch vụ --}}
                <div id="service-hidden-bag"></div>

                <div class="date-grid">
                    <div class="form-group">
                        <label class="form-label">Nhận phòng</label>
                        <input type="date" name="check_in" class="form-input" id="checkin-date" min="{{ date('Y-m-d') }}" required>
                    </div>
                    <div class="form-group">
                ...
                {{-- tổng dịch vụ (để route validate dùng) --}}
    ...
    <div class="reviews-section">
        <h2 class="info-title">Đánh giá từ khách hàng ({{ $totalReviews }})</h2>
        ...
        <div class="review-item">
            ...
            <p>{{ $review->comment ?? 'Khách hàng đã để lại đánh giá tích cực.' }}</p>
        </div>
        @endforeach
    </div>
    @endif
...
function recalcTotalsWithServices(){
    ...
    const base   = {{ (int)($roomType->base_price ?? 0) }};
