...
<section class="hero-slider-section">
  <div class="hero-swiper swiper">
    <div class="swiper-wrapper">
      @if(isset($latestAlbum) && $latestAlbum->count() > 0)
      @foreach($latestAlbum as $banner)
      <div class="swiper-slide">
        <img src="{{ asset('storage/' . $banner->images_path) }}" alt="{{ $banner->title }}" />
...
@endif
<form class="b-search-bar" action="{{ route('search') }}" method="post">
  @csrf
  ...
  <div class="b-search-field b-search-guests">
    ...
    <input
      ...
      value="{{ old('guests', request('guests', 2)) }}"
      min="1"
      max="10"
  ...
  <div class="offers-container">
    ...
    <div class="offer-card white-card">
      <div class="offer-info">
        ...
        <a href="{{ route('user.vouchers') }}" class="offer-button">Săn ưu đãi</a>
      </div>
      <img src="images/image2.jpeg" alt="Ưu đãi 1" />
    </div>

    <div class="offer-card blue-card">
      <img src="{{ asset('images/image1.jpg') }}" alt="Giới thiệu khách sạn" />
      <div class="offer-info white-text">
        ...
        <a href="{{ route('about') }}" class="offer-button white">Tìm hiểu thêm</a>
      </div>
    </div>
...
<section class="room-types-section">
  <div class="room-types-container">
    ...
    <div class="room-types-grid">
      ...
      <a href="{{ route('room_types.show', ['id' => $roomType->rt_id]) }}" class="room-type-card">
        <div class="room-type-image">
          <img src="{{ isset($roomType->main_image) ? asset($roomType->main_image) : asset('assets/images/default-room-type.jpg') }}"
        ...
        <div class="room-type-content">
          <div class="room-type-header">
            <h3 class="room-type-title">{{ $roomType->type_name }}</h3>
            ...
            <div class="room-type-rating">
              <span class="rating-badge">{{ number_format(rand(80, 95)/10, 1) }}</span>
              ...
              <span class="rating-count">({{ rand(150, 800) }} đánh giá)</span>
            </div>
          </div>
          ...
          <div class="room-type-features">
            <div class="room-type-specs">
              ...
              @if($roomType->number_beds)
              <span class="room-type-spec-item">
                <i class="fas fa-bed"></i>
                {{ $roomType->number_beds }} giường
              </span>
              @endif
            ...
            @if(isset($roomType->amenity_list) && count($roomType->amenity_list) > 0)
            <div class="room-type-amenities">
              <div class="room-type-amenities-list">
                @foreach(array_slice($roomType->amenity_list, 0, 3) as $index => $amenity)
                <span class="room-type-amenity-tag {{ $index == 0 ? 'featured' : '' }}">
                  {{ $amenity }}
                </span>
                @endforeach
                @if(count($roomType->amenity_list) > 3)
                <span class="room-type-amenity-tag">
                  +{{ count($roomType->amenity_list) - 3 }} tiện nghi
                </span>
                @endif
              </div>
            </div>
    ...
    <div class="room-types-view-more">
      <a href="{{ route('all_rooms') }}" class="room-types-view-more-btn">
        <i class="fas fa-th-large"></i>
        Hiển thị tất cả loại chỗ nghỉ
        @if(isset($stats['total_room_types']))
        ({{ $stats['total_room_types'] }})
        @endif
      </a>
    </div>
    @endif
